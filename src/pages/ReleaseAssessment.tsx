...
const ReleaseAssessment: React.FC = () => {
  ...
  const doExport = () => {
    const csv = exportReleaseToCSV(startDate, endDate, filterDistrict);
    downloadCSV(csv, `解矫名单_${startDate}_${endDate}`);
  };
    ...
    <div className="space-y-5">
      ...
      <div className="card overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500">共 {releaseList.length} 条记录，筛选条件：{startDate} 至 {endDate}{filterDistrict ? ` · ${filterDistrict}` : ''}</span>
        </div>
